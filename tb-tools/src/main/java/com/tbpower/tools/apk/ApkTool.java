...
public final class ApkTool {
    ...
    public void installApp(Context context, String filePath) {
        installApp(context, TBTools.file().getFileByPath(filePath));
    }
    ...
    public void installApp(Context context, File file) {
        if (!TBTools.file().isFileExists(file)) return;
        context.startActivity(TBTools.intent().getInstallAppIntent(file));
    }
    ...
    public void installApp(Activity activity, String filePath, int requestCode) {
        installApp(activity, TBTools.file().getFileByPath(filePath), requestCode);
    }
    ...
    public void installApp(Activity activity, File file, int requestCode) {
        if (!TBTools.file().isFileExists(file)) return;
        activity.startActivityForResult(TBTools.intent().getInstallAppIntent(file), requestCode);
    }
    ...
    public boolean installAppSilent(String filePath) {
        File file = TBTools.file().getFileByPath(filePath);
        if (!TBTools.file().isFileExists(file)) return false;
        String command = "LD_LIBRARY_PATH=/vendor/lib:/system/lib pm install " + filePath;
        ShellTool.CommandResult commandResult = TBTools.shell().execCmd(command, !isSystemApp(TBTools.app()), true);
    ...
    public void uninstallApp(Context context, String packageName) {
        if (TBTools.string().isEmpty(packageName)) return;
        context.startActivity(TBTools.intent().getUninstallAppIntent(packageName));
    }
    ...
    public void uninstallApp(Activity activity, String packageName, int requestCode) {
        if (TBTools.string().isEmpty(packageName)) return;
        activity.startActivityForResult(TBTools.intent().getUninstallAppIntent(packageName), requestCode);
    }
    ...
    public boolean uninstallAppSilent(Context context, String packageName, boolean isKeepData) {
        if (TBTools.string().isEmpty(packageName)) return false;
        String command = "LD_LIBRARY_PATH=/vendor/lib:/system/lib pm uninstall " + (isKeepData ? "-k " : "") + packageName;
        ShellTool.CommandResult commandResult = TBTools.shell().execCmd(command, !isSystemApp(context), true);
        return commandResult.successMsg != null && commandResult.successMsg.toLowerCase().contains("success");
    }
    ...
    public boolean isAppRoot() {
        ShellTool.CommandResult result = TBTools.shell().execCmd("echo root", true);
        if (result.result == 0) {
            return true;
        }
        if (result.errorMsg != null) {
            TBTools.log().d("isAppRoot", result.errorMsg);
        }
        return false;
    }
    ...
    public void launchApp(Context context, String packageName) {
        if (TBTools.string().isEmpty(packageName)) return;
        context.startActivity(TBTools.intent().getLaunchAppIntent(context, packageName));
    }
    ...
    public void launchApp(Activity activity, String packageName, int requestCode) {
        if (TBTools.string().isEmpty(packageName)) return;
        activity.startActivityForResult(TBTools.intent().getLaunchAppIntent(activity, packageName), requestCode);
    }
    ...
    public void getAppDetailsSettings(Context context, String packageName) {
        if (TBTools.string().isEmpty(packageName)) return;
        context.startActivity(TBTools.intent().getAppDetailsSettingsIntent(packageName));
    }
    ...
    public String getAppName(Context context, String packageName) {
        if (TBTools.string().isEmpty(packageName)) return null;
        try {
    ...
    public String getAppSignatureSHA1(Context context, String packageName) {
        ...
        return TBTools.secure().encryptSHA1ToString(signature[0].toByteArray()).
